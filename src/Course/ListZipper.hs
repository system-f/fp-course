...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper ls x rs) =
...
findLeft p z@(ListZipper _ x _) =
  if p x
    then 
      ...
      -- asMaybeZipper (findLeft p) (moveLeft z)
      -- findLeft p `asMaybeZipper` moveLeft z
      findLeft p -<< moveLeft z
...
moveLeft (ListZipper Nil _ _) =
  IsNotZ
moveLeft (ListZipper (h :. t) x rs) = 
  IsZ (ListZipper t h (x :. rs))
...
moveRight (ListZipper _ _ Nil) =
  IsNotZ
moveRight (ListZipper l x (r:.rs)) =
  IsZ (ListZipper (x:.l) r rs)
...
swapLeft (ListZipper Nil _ _) =
  IsNotZ
swapLeft (ListZipper (l:.ls) x r) =
  IsZ (ListZipper (x:.ls) l r)
...
swapRight (ListZipper _ _ Nil) =
  IsNotZ
swapRight (ListZipper l x (r:.rs)) =
  IsZ (ListZipper l r (x:.rs))
...
instance Extend ListZipper where
  ...
  (<<=) g z =
    ...
    -- error "rights" :: List b
    -- g z :: b
    -- unfoldr :: (x -> Optional (y, x)) -> x -> List y
    -- toOptional :: MaybeListZipper a -> Optional (ListZipper a)
    -- 
    -- unfoldr :: (ListZipper a -> Optional (b, ListZipper a)) -> ListZipper a -> List b
    -- zz :: ListZipper a
    -- moveLeft zz :: MaybeListZipper a
    -- toOptional (moveLeft zz) :: Optional (ListZipper a)
    -- ? :: Optional (b, ListZipper a)
    -- <$> :: (a -> b) -> f a -> f b
    -- <$> :: (a -> b) -> Optional a -> Optional b
    let wibble move = unfoldr (\zz -> (<$>) (\u -> (g u, u)) ((toOptional . move) zz)) z 
    in ListZipper 
         (wibble moveLeft) 
         (g z) 
         (wibble moveRight)
...
data Hole = Hole
-- produce a zipper of all possible zippers.
duplicate ::
  ListZipper a
