...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\a b -> helper (f a) a b) Nil
  where helper :: Bool -> a -> List a -> List a
        helper True x acc  = x :. acc
        helper False _ acc = acc
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight helper (Full Nil)
  where helper :: Optional a -> Optional (List a) -> Optional (List a)
        helper Empty _             = Empty
        helper _ Empty             = Empty
        helper (Full a) (Full acc) = Full (a :. acc)
...
lengthGT4 ::
  ...
  -> Bool
lengthGT4 = go 0
  where go :: Int -> List a -> Bool
        go 5 _            = True
        go _ Nil          = False
        go count (_ :. t) = go (count + 1) t
