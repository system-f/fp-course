...
foldRight f b (h :. t) = f h (foldRight f b t)
 ...
 -- fold right == constructor replacement
 -- fold left == for loop
...
filter ::
  ...
  -> List a
filter f =
  foldRight (\a bs -> if f a then a :. bs else bs) Nil
...
seqOptional ls =
  if length 
    ...
    ls) > 0 
    then Empty 
  else Full (foldRight (:.) (Nil) (map (\(Full a) -> a) ls))
...
find ::
  ...
  -> Optional a
find f =
  foldRight (\a o -> if f a then Full a else o) Empty
