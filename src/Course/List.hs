...
{-
getIt :: Optional Int -> Optional Int -> Int
getIt x y = case (x, y) of
            (Empty, Empty) -> 0
            (Full n, Empty) -> n
            (Empty, Full n) -> n
            (Full n, Full m) -> n + m
...
find ::
  ...
  -> Optional a
find p =
  foldRight (\a b -> if p a then Full a else b) Empty
