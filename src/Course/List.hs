...
filter ::
  ...
  -> List a
  -> List a
filter p = foldRight (\a bs -> ifThenElse (p a)  (a :. bs)  bs) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight fillIfFull (Full Nil)
  where fillIfFull Empty _ = Empty
        fillIfFull _ Empty = Empty
        fillIfFull (Full a) (Full as) = Full (a :. as)
