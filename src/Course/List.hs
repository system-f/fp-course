...
length ::
  ...
  -> Int
length =
  -- foldLeft (\r _ -> succ r)       0 x
     -- foldLeft (\r -> \_ -> succ r)       0 x
     
     -- foldLeft (\r -> konst (succ r)) 0 x
     -- foldLeft (\r -> (const . succ) r) 0 x
...
filter _ Nil =
  Nil
-- FP is DRY (taken seriously)
filter f (h:.t) =
  -- iifL id ((:.) h) (f h) (filter f t)
  -- (if f h then ((:.) h) else id) (filter f t)
  {-
  if f h
    then h :. filter f t
    else filter f t
