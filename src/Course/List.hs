...
flatMap f =
  ...
  -- foldRight (\a listb -> f a ++ listb) Nil list
-- replace (:.) with f then (++)
--         Nil with Nil  
...
-- List (t a) -> t (Optional a)
-- as long as t supports some notion
-- of "bind" and "Full"
...
-- []
--
-- prop> forall x y. let notReverse x ++ notReverse y == notReverse (y ++ x)
--
-- prop> forall x. let notReverse (x :. Nil) == x :. Nil
notReverse ::
  List a
  -> List a
