...
instance Monad Parser where
  ...
  (=<<) k p =
    P (\input -> 
          onResult (parse p input) (\input2 a -> parse (k a) input2))
      ...
      {-case parse p input of
                    ...
                    Result input2 a -> parse (k a) input2
                    )
-}
{-
onResult ::
...
-- \pred -> character >>= \c -> lift3 bool unexpectedCharParser pure pred c
