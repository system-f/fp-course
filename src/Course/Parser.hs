...
natural ::
  Parser Int
natural =
  bindParser (\d -> case read d of
                      Empty -> failed
                      Full a -> valueParser a) (list digit)
...
sequenceParser (p:.ps) =
  ...
  flbindParser p (\a ->
  -- as <- sequenceParser ps
  -- from as in sequenceParser ps
  flbindParser (sequenceParser ps) (\as ->
  valueParser (a :. as)))
-}
...
surnameParser ::
  Parser Chars
surnameParser =
  (\u -> (++) . (u :.)) <$> upper <*> thisMany 5 lower <*> list lower
