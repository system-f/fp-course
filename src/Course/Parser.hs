...
-- list1 p = get a from p, then as from (list p), then cons a:.as 
...
list1 p =
  ...
  do a <- p
     ...
     -}
{-
  flbindParser p (\a -> -- run p to produce a
    flbindParser (list p) (\as -> -- run list of p to produce as
      valueParser (a:.as)))
-}
...
satisfy k =
  character >>= 
--    if k c then valueParser c else unexpectedCharParser c
--    iff (unexpectedCharParser c) (valueParser c) (k c)
    lift3 iff unexpectedCharParser valueParser k
...
natural =
  ...
  list1 digit >>=
  optional failed valueParser . read
...
--  (\c d e -> c :. d ++ e) <$> upper <*> thisMany 5 lower <*> list lower
  ...
  {-
  -- run upper-case to produce c
  upper >>= \c ->
  -- run exactly 5 lower to produce d
  thisMany 5 lower >>= \d ->
  -- run 0 or more lower to produce e
  list lower >>= \e ->
  pure (c :. d ++ e)
-}

{-
  -- produce c from upper-case character
  do c <- upper
  -- produce d from exactly 5 lower character
     d <- thisMany 5 lower
  -- produce e from 0 or more lower character
     e <- list lower
     pure (c :. d ++ e)
  -}
...
phoneParser =
  {-
  -- run a digit, call it d
  digit >>= \d ->
  -- run a phone body, call it b
  phoneBodyParser >>= \b ->
  -- run a hash, call it nothing (drop it)
  is '#' >>= \_ ->
  return (d :. b)
  -}
  {-
...
personParser =
  ...
  -- run age, call it a
  ageParser >>= \a ->
  -- 1 or many spaces (drop)
  spaces1 >>>
  -- run first name, call it n
  firstNameParser >>= \n ->
  -- 1 or many spaces (drop)
  spaces1 >>>
  -- run surname, call it s
  surnameParser >>= \s ->
  -- 1 or many spaces (drop)
  spaces1 >>>
  -- run smoker, call it k
  smokerParser >>= \k ->
  -- 1 or many spaces (drop)
  spaces1 >>>
  -- run phone, call it p
  phoneParser >>= \p ->
  pure (Person a n s k p)
  -}
  {-
  do a <- ageParser
     spaces1
