...
list1 p =
  ...
  -- 0 or many p (call it b) and then
  -- always succeed with (cons them (a :. b))
...
satisfy ::
  ...
  -> Parser Char
satisfy =
  -- \p -> flbindParser character (
  -- lift3 bool unexpectedCharParser valueParser p)
  flbindParser character . lift3 bool unexpectedCharParser valueParser
...
-- \c -> bool (unexpectedCharParser c) (valueParser c) (p c))

-- (a -> b -> c -> d) -> (Char -> a) -> (Char -> b) -> (Char ->c) -> Char -> d

-- bool :: x           -> x           -> Bool           -> x
--      :: (Char -> x) -> (Char -> x) -> (Char -> Bool) -> Char -> x
...
firstNameParser ::
  Parser Chars
firstNameParser =
  -- upper (call it u) and then 
  -- 0 or many lower (call it v) and then
  -- always succeed with (u :. v)
  (:.) <$> upper <*> list lower
...
surnameParser =
  -- upper (call it u) and then
  -- exactly 5 lower (call it v) and then
  -- 0 or more lower (call it w) ann then
  -- always succeed with (u :. v ++ w)
