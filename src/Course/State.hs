...
instance Bind Idy where
  f =<< Idy a =
    f a

instance Monad Idy where
...
firstRepeat ::
  ...
  -> Optional a
firstRepeat list =
  -- \a -> if a is in the set then we have found it else
  -- put the a in the set and we have not found it
  -- s :: Set a
  -- a :: a
  ----
  -- ? :: (Bool, Set a)
