...
instance Applicative (State s) where
  ...
  (<*>) ::
    ...
    -> State s b 
  (<*>)  =
    \f a -> State (\s -> let (f', t) = runState f s
                         in  mapfst f' (runState a t))
...
distinct ::
  ...
  -> List a
distinct x =
  eval (filtering (\a -> State (\s -> (S.notMember a s, S.insert a s))) x) S.empty
